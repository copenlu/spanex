...
class ScalarMix(torch.nn.Module):
    ...
    def forward(self, tensors: List[torch.Tensor], mask: torch.BoolTensor = None) -> torch.Tensor:
        ...
        When `do_layer_norm=True`, the `mask` is required to be input.  If the `tensors` are
        ...
        normed_weights = softmax(torch.cat([parameter for parameter in self.scalar_parameters]), dim=0)
